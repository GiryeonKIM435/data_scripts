function r = fitKrTrailingTimeBandCascade(defC, forceC, secC, yieldInfo, methodDef, fitCfg)
%fitKrTrailingTimeBandCascade [t−n−w, t−n) の時間 trailing 帯域 kr

if nargin < 6 || isempty(fitCfg)
    fitCfg = struct();
end
...
if nargin < 3 || isempty(secC)
    secC = nan(size(defC));
else
    secC = secC(:);
    secC = secC(idxC:min(idxC + numel(defC) - 1, numel(secC)));
end
